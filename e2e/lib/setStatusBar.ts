...
export async function setStatusBar() {
  if (device.getPlatform() === 'ios') {
    ...
    execSync('xcrun simctl status_bar booted override --time 9:41 --batteryState charged --batteryLevel 100 --wifiBars 3 --cellularMode active --cellularBars 4');
  } else {
    // enter demo mode
    execSync('adb shell settings put global sysui_demo_allowed 1');
    // display time 6:00
    execSync('adb shell am broadcast -a com.android.systemui.demo -e command clock -e hhmm 1850');
    // Display full mobile data with 4g type and no wifi
    execSync('adb shell am broadcast -a com.android.systemui.demo -e command network -e mobile show -e level 4 -e datatype 4g -e wifi false');
    // Hide notifications
