...
export function getOutputDir(projectRoot: string, locale: string) {
  return path.resolve(projectRoot, `meta/screenshots/${device.getPlatform()}/${locale}/`);
}

export async function takeScreenshotAsync({ info, projectRoot, index, locale }: { projectRoot: string; locale: string; info?: string; index?: number }) {
...
export function createCapture({ projectRoot, locale }: { locale: string; projectRoot: string }) {
  const captureAsync = () =>
    takeScreenshotAsync({
      projectRoot,
      locale,
